...
This page covers Arduino IDE setup and board-specific notes for the **Tarangify T-CORE-S3-BAT** development board.
...
| Input voltage | 3.7V Li-ion/LiPo battery via battery connector, plus 5V via USB |
...
**Power limits:** Use a compatible 3.7V Li-ion/LiPo battery with the board's battery connector. Do not connect a 5V–28V external supply directly to the battery input, as this may damage the board. For USB power, use a standard regulated 5V USB supply.

:::

:::tip

The battery and USB power sources can be used with the board as intended. When using USB, ensure the battery is connected according to the board's charging and power-management design.
...
2. Search for and select **ESP32S3 Dev Module** — T-CORE-S3-BAT doesn't currently have a dedicated entry in the boards list, so this generic profile is the correct starting point.
